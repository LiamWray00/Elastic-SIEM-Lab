...
## Lesson learned
The Kibana UI's Saved Objects export doesn't work for detection rules (they're excluded by type, discovered on Day 3) — but the Detection Engine's own REST API exports them cleanly and can be automated. This is a more realistic, repeatable way to manage rules-as-config than manual UI export, and is the kind of small script a real detection engineering workflow would use for backups, version control, or CI/CD rather than relying on manual clicks. Also reinforced a security habit: always check exported/generated files for embedded secrets before publishing, and never commit real API keys to a public repo — use placeholders.
