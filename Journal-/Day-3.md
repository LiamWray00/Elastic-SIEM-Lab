...
Detection (a rule firing) and investigation (understanding what actually happened) are different skills supported by different tooling. Elastic Security's Timeline pulls in correlated context automatically from an alert and converts raw process/network events into readable narratives, which is closer to what a SOC analyst experiences during triage rather than just reading an alert's summary line. I also learned that alerts can silently drop out of view under narrow default time filters; it's worth checking both the time range and status tabs (Open/Acknowledged/Closed) before assuming data is missing.
