...
- **Different rule types fit different signals.** A single occurrence of `nmap` running is inherently suspicious (Custom query rule fits). A single `sshd` process is normal; only a burst is suspicious (the Threshold rule fits). Picking the right rule type is crucial for the detection design.
